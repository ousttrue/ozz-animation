...
bool OZZ_load_mesh(ozz_t *p, const void *ptr, size_t size, vertex_t **vertices,
                   int *num_vertices, uint16_t **indices,
  ...
  *vertices = (vertex_t *)malloc(*num_vertices * sizeof(vertex_t));
  ...
  *indices = (uint16_t *)malloc(*num_triangle_indices * sizeof(uint16_t));
...
void OZZ_update_joints(ozz_t *p, int num_instances, float abs_time_sec,
  ...
  for (int instance = 0; instance < num_instances; instance++) {
    ...
    for (int i = 0; i < p->joint_remaps.size(); i++) {
      auto skin_matrix =
